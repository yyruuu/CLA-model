...
mpl.use('TkAgg')
faulthandler.enable()
def rnn_model(step, input_dim):
    """
    step: 步长
    input_dim：输入维度
    """
    inputs = Input(shape=(step, input_dim))
    lstm_out = SimpleRNN(24, return_sequences=True, activation='elu')(inputs)
    lstm_out = SimpleRNN(24, return_sequences=True, activation='elu', dropout=0.2)(lstm_out)
